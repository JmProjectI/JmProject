...
{
    public class Exp_TracesBLL
    {
        DBHelperSql dao = new DBHelperSql();
        public Exp_TracesBLL()
        { }

        public int Insert(Exp_Traces model)
        {
            return dao.Insert<Exp_Traces>(model);
        }
        public int Update(Exp_Traces model)
        {
            return dao.Update<Exp_Traces>(model);
        }
        public int Delete(String id)
        {
            return dao.Delete("delete from Exp_Traces where ='" + id + "'");
        }
        public string Maxid()
        {
            string id = "";
            String tsql = "select max() from Exp_Traces";
        ...
        {
            string id = "";
            String tsql = "select max() from Exp_Traces where ID Like '" + D + "%'";
            string result = dao.GetScalar(tsql).ToStringEx();
            if (result == "")
            {
                id = D + "0001";
        ...
        public bool isExist(String _where)
        {
            ...
            String tsql = "select count(*) from Exp_Traces" + where;
        ...
        {
            String where = " where 1=1 " + _where;
            String tsql = "select " + fieldName + " from Exp_Traces" + where;
            object result = dao.GetScalar(tsql);
            return result.ToStringEx();
        }
        public List<Exp_Traces> SelectAll(string Where, GridPager pager)
        {
            string Order = string.Empty;
            string Table = "Exp_Traces";
            string Fields = "[LogisticCode],[AcceptTime],[AcceptStation],[Remark]";
            ...
            return dao.ProExecSelect<Exp_Traces>("Proc_Page", sp);
        }
        public Exp_Traces GetRow(Exp_Traces model)
        {
            return dao.GetRow<Exp_Traces>(model);
        }
        ...
        {
            return GetData(fieldsName, _where, "Exp_Traces");
        }
