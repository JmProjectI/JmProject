...
function InitWebUpload() {
    ...
    var uploader = WebUploader.create({
        ...
        // 文件接收服务端。
        server: applicationPath + 'Sale/UploadOrderXls',
        // 选择文件的按钮。可选。
    ...
    uploader.on('uploadSuccess', function (file, response) {
        $.post('/Sale/MergeOrderXls', { guid: GUID, fileName: file.name }, function (data) {
            if (data.type == 0) {
                ...
                $("#ddlSheetName").combobox({
                    url: '/Sale/GetOExcelSheetName?filename=' + filename,
                    valueField: 'id',
                    textField: 'name'
                });
